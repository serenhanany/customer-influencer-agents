...
- `customer-agent` and `influencer-agent` are both wired to a real LLM (NVIDIA NIM via NAT) as of Week 3 — see each service's own `README.md` for details. `influencer-agent` has a `poller.py` that autonomously watches `social_network` and reacts; `customer-agent` now has an equivalent driver (`driver.py`, added alongside the `event-generator` service) that subscribes to crisis events and reacts automatically, once per persona, in addition to remaining invocable on demand as an MCP tool. Verified end to end, including real ticket creation via `open_support_ticket`. Note: the on-demand and autonomous paths currently run as separate workflow instances with independent in-memory persona state — see `customer-agent/README.md`'s "What's not done yet".